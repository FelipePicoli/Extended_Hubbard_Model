...
let
    ...
    # Checks if case was already run.
    info_output = @sprintf("XXXXX_%s_L=%d_U=%.2f_V=%.2f_NPoints=%d.txt", model, L, U, V, Npoints)
    fname = joinpath(results, replace(info_output, "XXXXX" => "E_GS"))
    if isfile(fname)
        @sprintf("E_p with : L=%d, U=%.2f, V=%.2f already computed. Skipping job...", L, U, V)
    else 
        @sprintf("Running simulation for L=%d, U=%.2f, V=%.2f.", L, U, V)
